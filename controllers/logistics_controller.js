...
const getSent = async (req, res) => {
    try {
        const query = "SELECT Logistics.id, Logistics.[from], Logistics.[to], Logistics.received, Logistics.[type], ProductModels.product_line, ProductModels.name AS model FROM Logistics JOIN LotLogistics ON Logistics.id = LotLogistics.delivery_id JOIN Lots ON Lots.id = LotLogistics.lot_number JOIN ProductModels ON Lots.model = ProductModels.id WHERE Logistics.[from] = :user_id UNION SELECT Logistics.id, Logistics.[from], Logistics.[to], Logistics.received, Logistics.[type], ProductModels.product_line, ProductModels.name AS model FROM Logistics JOIN IndividualLogistics ON IndividualLogistics.delivery_id = Logistics.id JOIN Products ON IndividualLogistics.product_id = Products.id JOIN Lots ON Lots.id = Products.lot_number JOIN ProductModels ON Lots.model = ProductModels.id WHERE Logistics.[from] = :user_id"
        const result = await sequelize.query(query, {
            replacements: { user_id: req.user.id },
            type: QueryTypes.SELECT
        })
